...
public class SipService extends BackgroundService implements SipServiceConstants {
    ...
    private static final String TAG = SipService.class.getSimpleName()+"_tlq";
    ...
    private void handleMakeCall(Intent intent) {
        ...
        try {
            ...
            Logger.debug(TAG, "handleMakeCall callId = " +callId);
            mBroadcastEmitter.outgoingCall(accountID, callId, number, isVideo, isVideoConference, isTransfer);
        } catch (Exception exc) {
...
//                ex.printStackTrace();
    ...
    private void loadNativeLibraries() {
        try {
            System.loadLibrary("c++_shared_legrand");
            Logger.debug(TAG, "libc++_shared_legrand loaded");
        } catch (UnsatisfiedLinkError error) {
            Logger.error(TAG, "Error while loading libc++_shared_legrand native library", error);
            throw new RuntimeException(error);
        }

        try {
            System.loadLibrary("openh264_legrand");
            Logger.debug(TAG, "OpenH264_legrand loaded");
        } catch (UnsatisfiedLinkError error) {
        ...
        try {
            System.loadLibrary("pjsua2_legrand");
            Logger.debug(TAG, "PJSIP pjsua2_legrand loaded");
        } catch (UnsatisfiedLinkError error) {
            Logger.error(TAG, "Error while loading PJSIP pjsua2_legrand native library", error);
            throw new RuntimeException(error);
        }
    }
    ...
    private void startStack() {
        ...
        try {
            Logger.debug(TAG, "startStack Starting PJSIP");
            mEndpoint = new SipEndpoint(this);
            try {
                mEndpoint.libCreate();
            } catch (Exception e) {
                Logger.error(TAG, "startStack libCreate e="+e.getMessage());
            }
...
//            tlsTransport.setPort(8897);
            ...
            Logger.debug(TAG, "startStack PJSIP end!");
        } catch (Exception exc) {
            Logger.error(TAG, "startStack Error while starting PJSIP e="+ exc.getMessage());
            mStarted = false;
        }
    }
    ...
    private void addAccount(SipAccountData account) throws Exception {
        ...
        if (sipAccount == null || !sipAccount.isValid() || !account.equals(sipAccount.getData())) {
            ...
            try {
                ...
                Logger.error(TAG, "sipAccount.setRegistration(true) = " + getValue(getApplicationContext(), account.getIdUri()));
